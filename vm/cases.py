...
async def main():
    ...
    completed, pending = await asyncio.wait(co)
...
loop = asyncio.get_event_loop()
loop.run_until_complete(asyncio.wait([main("A"), main("B"), main("C")]))
s = sorted(results)
...
loop = asyncio.get_event_loop()
loop.run_until_complete(asyncio.wait([main("A", 1), main("B", 2), main("C", 3)]))
s = sorted(results)
...
c = __annotations__
print(sorted(c.items()))
assert(c == {'a': typing.List[int], 'b': str})
"""),
